...
const RegisterPage = () => {
    ...
    const [credentials, setCredentials] = useState({ fullName: "", username: "", email: "", password: "" });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setIsError(true);
            setErrorMessage(err.response?.data?.message || "An error occurred during register");
        } finally {
            setIsPending(false);
        }
    };
            ...
            <div className='flex flex-col justify-center items-center'>
                <form className='flex gap-4 flex-col w-[350px]' onSubmit={handleSubmit}>
                    <div className="avatar mx-auto">
                        <div className="w-36 h-36 rounded-full ring ring-[#FC852E] ring-offset-base-100 ring-offset-1 cursor-pointer" onClick={() => profileImgRef.current.click()}>
                            <img src={profilePhoto || "/avatar-placeholder.jpg"} alt="profile-photo" />
                        </div>
                    </div>
                    <input
                        type="file"
                ...
                <div className='mt-2'>
                    <p className='text-white text-lg'>Already have an account? <Link to='/login' className="text-[#FC852E] hover:underline">
                        Login
                    </Link></p>
                </div>
            </div>
