...
const UserProfile = () => {
  ...
  useEffect(() => {
    // Fetch states and cities from an API or define them statically
    const fetchStatesAndCities = async () => {
      try {
        const statesRes = await axios.get("/path-to-states-api"); // Replace with your API endpoint
        setStateOptions(statesRes.data);

        if (state) {
          const citiesRes = await axios.get(
            `/path-to-cities-api?state=${state}`
  ...
  const handleUpdate = async () => {
    try {
      ...
      toast.error(
        err.response?.data?.message || "An error occurred during profile update"
      );
    }
  };
  ...
  return (
    <div className="relative min-w-[400px] border-r-[1px] border-gray-700 flex flex-col">
      <div className="w-full border-b-[1px] border-gray-700 flex items-center px-2 py-2">
        <button
          className="btn btn-ghost btn-circle btn-sm w-10 h-10"
          onClick={handleBack}
        >
      ...
      <div className="flex-1 flex items-center flex-col py-10 px-8">
        {canEdit ? (
          ...
          <img
            src={profileData?.profilePhoto || `/avatar-placeholder.jpg`}
            className="rounded-full w-40 h-40 object-cover"
          />
        )}
        <input
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {nameInput && (
              <input
                ...
                className="outline-none bg-transparent border-b-2 border-[#FC852E] py-1 text-lg font-semibold w-full"
              />
            )}
            {canEdit &&
              (nameInput ? (
                <button
                  className="btn btn-circle btn-ghost btn-sm w-10 h-10"
                  onClick={handleUpdate}
                >
                  <MdDone size={22} />
                </button>
        ...
        <div className="w-full mt-6 flex gap-4">
          <div className="w-1/2">
            <p className="text-[#FC852E] text-sm">State</p>
            <span className="text-lg font-semibold">{state}</span>
          </div>
          <div className="w-1/2">
            <p className="text-[#FC852E] text-sm">City</p>
